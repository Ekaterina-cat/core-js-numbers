...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const angleInDegrees =
    ((x1 * x2 + y1 * y2) / Math.abs(Math.sqrt(x1 ** 2 + y1 ** 2))) *
    Math.abs(Math.sqrt(x2 ** 2 + y2 ** 2));
  return Math.acos(angleInDegrees);
}
...
function getSumOfDigits(num) {
  const initValue = 0;
  const arrNum = [...num.toString()].map(Number);
  return arrNum.reduce((acc, current) => acc + current, initValue);
}
